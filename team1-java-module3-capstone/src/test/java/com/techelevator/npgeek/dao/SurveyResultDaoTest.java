...
public class SurveyResultDaoTest {
	...
	@Test
	public void testGetAllSurveryResults() {
		List<SurveyResult> surveyResults = dao.getAllSurveyResults();
		
		assertEquals(8,surveyResults.size());
		
		SurveyResult oneSurveyResult = null;
		for(SurveyResult surveyResult : surveyResults) {
			if(surveyResult.getEmailAddress().equals("monster@monster.com")) {
				oneSurveyResult = surveyResult;
			}	
		}
		
		assertEquals("GTNP",oneSurveyResult.getParkCode());
		assertEquals("monster@monster.com",oneSurveyResult.getEmailAddress());
		assertEquals("Pennsylvania",oneSurveyResult.getState());
		assertEquals("sedentary",oneSurveyResult.getActivityLevel());
	}
	
	
	
	@Test
	public void testAddSurveyResult() {
		
		SurveyResult newSurveyResult = new SurveyResult();
		newSurveyResult.setParkCode("YNP2");
		newSurveyResult.setEmailAddress("soccerdoccer@soccerdoccer.com");
		newSurveyResult.setState("Kentucky");
		newSurveyResult.setActivityLevel("extremely active");
		
		dao.addSurveyResult(newSurveyResult);
		
		
		assertEquals(9,dao.getAllSurveyResults().size());
		
		List<SurveyResult> surveyResults = dao.getAllSurveyResults();
		SurveyResult oneSurveyResult = null;
		for(SurveyResult surveyResult : surveyResults) {
			if(surveyResult.getEmailAddress().equals("soccerdoccer@soccerdoccer.com")) {
				oneSurveyResult = surveyResult;
			}	
		}
		
		assertEquals("YNP2",oneSurveyResult.getParkCode());
		assertEquals("soccerdoccer@soccerdoccer.com",oneSurveyResult.getEmailAddress());
		assertEquals("Kentucky",oneSurveyResult.getState());
		assertEquals("extremely active",oneSurveyResult.getActivityLevel());
	...
	@Test
	public void testGetParkRankings() {
		Map<String,Integer> parkRankings = dao.getRankedParks();
		assertEquals(5,parkRankings.size());
		String outputString = "";
		for(String key : parkRankings.keySet()) {
			outputString += key + "\t" + parkRankings.get(key) + "\n";
		}
		assertEquals("Unexpected Data Returned","MRNP\t3\nGTNP\t2\nCVNP\t1\nENP\t1\nGSMNP\t1\n",outputString);
	}
	
	
	@Test
	public void testGetSubmittedEmails() {
		List<String> submittedEmails = dao.getSubmittedEmails();
		assertEquals("Wrong number of entries in returned list.",7,submittedEmails.size());
		assertEquals("Unexpected email address","alix@alix.com",submittedEmails.get(0));
		assertEquals("Unexpected email address","billycat@billycat.com",submittedEmails.get(1));
		assertEquals("Unexpected email address","bug@bug.com",submittedEmails.get(2));
		assertEquals("Unexpected email address","monster@monster.com",submittedEmails.get(3));
		assertEquals("Unexpected email address","t@t.com",submittedEmails.get(4));
		assertEquals("Unexpected email address","thomas@thomas.com",submittedEmails.get(5));
		assertEquals("Unexpected email address","tim@timnevits.com",submittedEmails.get(6));
	}
